...
> 最近全量验收：2026-08-31 通过（自动化回归 16/16 + 人工项全过）
...
### 基础检查
- [ ] 扩展图标显示在浏览器工具栏
- [ ] 点击图标弹出配置面板，显示 8 个开关 + 「导出为 Markdown」按钮
- [ ] 默认开关状态: 复制开、右键开、选择开、水印开、拖拽关、表格格式开、导出内嵌图片开、调试关
...
| 3 | 关闭开关 | 右键无反应或被拦截（经配置桥接即时生效，无需刷新） |
| 4 | 重新开启 | 即时恢复，无需刷新 |
...
| 4 | 关闭开关，刷新 | MAIN world 与 ISOLATED world 均无 [飞书复制助手] 日志 |

### 2.8 图片复制到剪贴板

| 步骤 | 操作 | 期望结果 |
|------|------|----------|
| 1 | 在正文图片上右键 → 复制图片 | 浏览器原生菜单出现，点击后剪贴板写入图片 |
| 2 | 粘贴到微信/画图/飞书文档 | 图片正常显示 |
| 3 | 单击图片打开预览灯箱，在灯箱图片上右键 → 复制图片 | 同样生效 |
| 4 | 右键 → 图片另存为 | 图片下载到本地 |

> 实现机制：contextmenu 解禁后浏览器原生菜单天然支持，blob: 与 drive-stream URL 图片均适用（2026-08-31 实测通过）。

### 2.9 导出为 Markdown

| 步骤 | 操作 | 期望结果 |
|------|------|----------|
| 1 | 「导出包含图片」开启，Popup 点击「导出为 Markdown」 | 页面自动滚动收集后下载 `文档名.zip` |
| 2 | 解压 zip 检查 | 根目录含 `文档名.md` + `assets/` 文件夹（img-001.png 等），md 内图片为 `assets/img-xxx.png` 相对路径 |
| 3 | 用 Typora/VSCode 打开 md | 图片正常显示（相对路径生效） |
| 4 | 「导出包含图片」关闭时导出 | 仅下载 .md，图片为原始 URL 链接 |
| 5 | 含失效图片的文档导出 | 失败图片保留原始 URL，不阻断整体导出 |

> 注：「选中复制为 Markdown」快捷键方案已放弃（2026-08-31）；base64 内嵌方案已替换为 zip + assets（2026-08-31，base64 文件体积过大）。

### 2.10 恢复原生保存

| 步骤 | 操作 | 期望结果 |
|------|------|----------|
| 1 | 文档页面按 Ctrl+S / Cmd+S | 弹出浏览器另存为对话框 |
| 2 | 按 Ctrl+P / Cmd+P | 弹出打印对话框，可存为 PDF |

> 已知限制：保存的 HTML / 打印 PDF 中图片因飞书加密+鉴权不可见。

### 2.11 外链新标签打开

| 步骤 | 操作 | 期望结果 |
|------|------|----------|
| 1 | 点击文档中的域外链接（非 feishu.cn/larksuite.com/larkoffice.com） | 直接新标签打开目标站，不经过飞书安全跳转页 |
| 2 | 点击飞书域内链接（知识库内其他文档） | 正常域内跳转，不受影响 |
...
| 文档类型 | 普通复制 | 表格复制 | 右键 | 文本选择 | 水印 | 图片复制 | Markdown |
|----------|----------|----------|------|----------|------|----------|----------|
| 普通文档 | 待测试 | N/A | 待测试 | 待测试 | 待测试 | 待测试 | 待测试 |
| 表格文档 | 待测试 | 待测试 | 待测试 | 待测试 | 待测试 | N/A | 待测试 |
| 知识库文档 | 待测试 | 待测试 | 待测试 | 待测试 | 待测试 | 待测试 | 待测试 |
| 思维笔记 | 待测试 | N/A | 待测试 | 待测试 | 待测试 | N/A | N/A |
| 多维表格 | 待测试 | 待测试 | 待测试 | 待测试 | 待测试 | N/A | N/A |
...
5. [ ] 水印隐藏
6. [ ] 表格复制格式保留
6.1 [ ] 图片可复制到剪贴板（或失败时降级下载）
6.2 [ ] 导出 Markdown 文件内容正确
6.3 [ ] Ctrl+S 弹出另存为、Ctrl+P 弹出打印
6.4 [ ] 连续拨动 3 个不同开关后刷新，各开关状态与显示一致（无互相重置）
7. 关闭「复制限制」，刷新
8. [ ] 复制被禁止（飞书原生行为）
9. 开启「复制限制」，刷新
10. [ ] 复制恢复
...
(async function runTests() {
  ...
  assert('XHR hook 存在',
    /permission/i.test(XMLHttpRequest.prototype.open.toString()));
  assert('CSS 已注入',
...
| 开关改了但行为没变 | MAIN world 未收到配置 | 刷新页面；确认配置桥接（postMessage）日志 |
| 拨一个开关其他开关复位 | SAVE_CONFIG 覆写 | 已修复则不应出现；检查 background merge 逻辑 |
| 图片复制变成下载文件 | ClipboardItem 写入失败的兜底 | 属预期降级；检查页面是否聚焦、剪贴板权限 |
| Ctrl+S 无反应 | 快捷键仍被飞书拦截 | 检查 keydown hook 是否在捕获阶段放行 |
