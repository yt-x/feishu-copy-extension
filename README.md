...
- 解除复制限制 — Ctrl+C、右键复制均可正常工作，富文本/表格格式保留
- 表格跨单元格复制 — 拖选多个单元格复制，粘贴到 Excel 按格落位
- 恢复右键菜单 — 检查元素、另存图片、复制链接等原生功能
- 图片复制/另存 — 右键菜单的「复制图片」「图片另存为」对正文与预览灯箱图片均有效
- 强制文本可选 — 覆盖 user-select CSS 限制
- 去除水印 — 隐藏页面上的水印图层
- 导出 Markdown — 控制面板一键导出整篇文档为 .md 文件
- 恢复原生保存 — Ctrl+S 另存为、Ctrl+P 打印为 PDF
...
4. 表格中拖选多个单元格后 `Ctrl+C`，粘贴到 Excel 保留单元格结构
5. 图片上右键使用「复制图片」/「图片另存为」
6. `Ctrl+S` 另存为、`Ctrl+P` 打印为 PDF
7. 控制面板点击「导出为 Markdown」下载整篇文档
...
确认「保留表格格式」已开启并刷新页面。跨单元格拖选复制已内置支持（通过选中态 overlay 重建表格）。

**保存的 HTML / 打印的 PDF 里图片不显示？**

已知限制：飞书图片加密且 URL 需登录态，保存物中的图片无法直接访问。图片本地化保存方案在后续路线图中。
